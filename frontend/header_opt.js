...
const headerRegex = /<div className="flex flex-wrap items-center gap-3">\s*<a href=\{`\/bio\/\$\{bioPage\.alias\}`\}[^>]*>.*?<\/a>\s*<button[^>]*>.*?Share QR\s*<\/button>\s*<button[^>]*>.*?Share\s*<\/button>\s*<\/div>/s;

const headerReplace = `<div className="flex items-center gap-2 w-full sm:w-auto mt-2 sm:mt-0">
...
console.log("Header replaced via regex in node!");
