        ...
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden sm:flex sm:items-center sm:gap-6">
            {isAdmin ? (
              ...
              <>
                {mainLinks.map(link => (
                  <Link key={link.href} href={link.href} className={`text-sm font-medium transition-colors h-16 flex items-center border-b-2 ${pathname === link.href ? 'text-[#1a73e8] border-[#1a73e8]' : 'text-[#5f6368] border-transparent hover:text-[#202124]'}`}>
                    {link.name}
                  </Link>
                ))}
                
                {/* More Dropdown */}
                <div className="relative h-16 flex items-center" ref={dropdownRef}>
                  <button 
                    onClick={() => setIsMoreDropdownOpen(!isMoreDropdownOpen)}
                    className={`text-sm font-medium flex items-center gap-1 transition-colors ${moreLinks.some(l => pathname === l.href) ? 'text-[#1a73e8]' : 'text-[#5f6368] hover:text-[#202124]'}`}
                  >
                    More
                ...
                <button onClick={handleLogout} className="text-sm font-medium text-[#5f6368] hover:text-[#d93025] transition-colors">
                  Logout
                </button>
              </>
            ) : (
